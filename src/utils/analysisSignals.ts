...
export interface AnalysisRiskWarning {
  code: string
  message: string
}
...
export interface AnalysisSignals {
  cacheHit: boolean
  cacheLabel: string
  riskWarnings: AnalysisRiskWarning[]
}

function pickObject(value: unknown): PlainObject | null {
  return value !== null && typeof value === 'object' && !Array.isArray(value)
    ? (value as PlainObject)
    : null
}

function toLabelSuffix(version: unknown): string {
  const text = String(version || '').trim()
  return text ? ` (${text})` : ''
}

export function resolveAnalysisSignals(rawStats: unknown): AnalysisSignals {
  ...
  const riskWarnings: AnalysisRiskWarning[] = []
