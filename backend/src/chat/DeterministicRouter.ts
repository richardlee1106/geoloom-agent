...
const AREA_ANALYSIS_VERB_RE = /(解读|分析|总结|梳理|判断|读懂|看看|看下|看一下|看一看)/u
const ANCHOR_LEAD_IN_RE = /^(?:请帮我看看|请帮我看下|请帮我看一下|请帮我|帮我看看|帮我看下|帮我看一下|帮我|请把|把|请快速|请直接|请|麻烦你|看看|看下|看一下|看一看|先看看|先看下|先看一下|解读一下|解读|分析一下|分析|总结一下|总结|梳理一下|梳理)\s*/u
...
function isAreaOverviewQuery(text: string) {
  return AREA_OVERVIEW_RE.test(text)
    || AREA_SEMANTIC_CONTEXT_RE.test(text)
    || (hasCurrentAreaReference(text) && AREA_ANALYSIS_VERB_RE.test(text))
}
